...
void calc_optical_flow_farneback(cv::Mat* from, cv::Mat* to, cv::Mat* out,
	...
	auto optical_flow = cv::FarnebackOpticalFlow::create(numLevels, pyrScale,
			fastPyramids, winSize, numIters, polyN, polySigma, flags);
...
void calc_optical_flow_dtvl1(cv::Mat* from, cv::Mat* to, cv::Mat* out,
		...
		double epsilon, int innerIterations, int outerIterations,
		double scaleStep, double gamma, int medianFiltering,
		bool useInitialFlow) {
	...
	auto optical_flow = cv::DualTVL1OpticalFlow::create(tau, lambda, theta,
			nscales, warps, epsilon, innerIterations, outerIterations,
			scaleStep, gamma, medianFiltering, useInitialFlow);
