...
void cvsys_cvt_color(cv::Mat* mat, cv::Mat* out, int code) {
    cv::cvtColor(*mat, *out, code);
}

void cvsys_pyr_down(cv::Mat* mat, cv::Mat* out) {
    cv::pyrDown(*mat, *out);
}
...
void cvsys_gaussian_blur(cv::Mat* mat, cv::Mat* out, Size2i ksize, double sigma_x, double sigma_y, int bordertype) {
...
void cvsys_calc_hist(const cv::Mat* images,
                     int nimages,
                     const int* channels,
                     cv::Mat* mask,
                     cv::Mat* hist,
